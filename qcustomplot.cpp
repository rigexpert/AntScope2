...
void QCPAxis::setRange(const QCPRange &range)
{
  ...
  if (range.lower < m_rangeMin)
  {
      mRange.lower = m_rangeMin;
      return;
  }
  if(range.upper > m_rangeMax)
  {
      mRange.upper = m_rangeMax;
      return;
  }
...
double QCPAxis::getRangeLower( void)
{
    return mRange.lower;
}

double QCPAxis::getRangeUpper( void)
{
    return mRange.upper;
}
...
void QCPAxis::setRangeMin(double min)
{
    m_rangeMin = min;
}
void QCPAxis::setRangeMax(double max)
{
    m_rangeMax = max;
}
...
void QCPAxis::scaleRange(double factor, double center)
{
  ...
  {
    QCPRange newRange;
    double low = (mRange.lower-center)*factor + center;
    double up = (mRange.upper-center)*factor + center;
    if(up < low + 1)
    {
        return;
    }
...
//              qDebug() << tmp;
          ...
          {
              int len = tmp.length() - index;
              if((tmp.length()-len) > 6)
              {
                  tmp.insert(tmp.length()-len-6," ");
              }
              if((tmp.length()-len) > 3)
              {
                  tmp.insert(tmp.length()-len-3," ");
              }
          }
...
QCPGraph::~QCPGraph()
{
//        delete mData; //If i use setData with false copy parameter,
                        //data deleting in other place
}
...
/*! \overload
  ...
  smallest vector.
*/
void QCPGraph::setData(const QVector<double> &key, const QVector<double> &value, int lenght)
{
      mData->clear();
      int n = lenght;
      //n = qMin(n, value.size());
...
void QCPGraph::getData()
{
    if(!mData->isEmpty())
    {
        QList <QCPData> vList = mData->values();
        QList <double> kList = mData->keys();
        for(int i = 0; i < 10; i++)
        {
            qDebug() << "key: " << kList.at(i);
            qDebug() << "value: " << vList.at(i).key;
        }
    }
    return;
}
...
QPointF QCPGraph::lowerFillBasePoint(double lowerKey) const
{
  ...
  if (valueAxis->scaleType() == QCPAxis::stLinear)
  {
    ...
    {
      point.setX(valueAxis->coordToPixel(20));// change by me from 0 to 20
      point.setY(lowerKey);
    } else if (keyAxis->axisType() == QCPAxis::atRight)
    {
      point.setX(valueAxis->coordToPixel(20));
      point.setY(lowerKey);
    } else if (keyAxis->axisType() == QCPAxis::atTop)
    {
      point.setX(lowerKey);
      point.setY(valueAxis->coordToPixel(20));
...
/*! \class QCPItemTriangle
  ...
  It has two positions, \a topLeft and \a bottomRight, which define the rect the Triangle will be drawn in.
*/

/*!
  Creates a Triangle item and sets default values.
...
/* inherits documentation from base class */
QPointF QCPItemTriangle::anchorPixelPoint(int anchorId) const
{
  QRectF rect = QRectF(topLeft->pixelPoint(), bottomRight->pixelPoint());
  switch (anchorId)
  {
